...
languageDef =
  emptyDef
    ...
    , Token.reservedNames =
        words
          "true false var if while fun ref return try catch reset shift spawn detach join"
...
program = do
  ...
  return $
    STry
      (SBlock (foldr SSeq SSkip ss))
      "__ex"
      (ECall
         (EVar "println")
         [EVal (VString "Uncaught exception: "), EVar "__ex"]
         [])
...
neg =
  (anyUnaOp (words "! -") >>= \o -> factor >>= \r -> return $ o r) <|> factor

factor = literal <|> fun <|> atomicOrCall <|> ref

literal =
  intLiteral <|> boolLiteral "false" False <|> boolLiteral "true" True <|>
  stringLiteral
